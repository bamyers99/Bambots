...
class AllPagesLister
{
     ...
     */
    public function __construct($mediawiki, $namespace = '0')
    {
    ...
    public function getNextBatch()
    {
        ...
        $ret = $this->mediawiki->getList('allpages', $params);

        if (isset($ret['error'])) throw new Exception('AllPagesLister.getNextBatch() failed ' . $ret['error']);
        if (isset($ret['continue'])) $this->continue = $ret['continue'];
        else $this->continue = false;

        return $ret['query']['allpages'];
    }
}