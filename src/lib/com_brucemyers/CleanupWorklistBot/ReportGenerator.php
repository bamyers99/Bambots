...
class ReportGenerator
{
	...
	function generateReports($project, $isWikiProject, $project_pages)
	{
		...
		$histurl = $this->urlpath . 'history/' . urlencode($filesafe_project) . '.html';
		$bycaturl = 'https://en.wikipedia.org/wiki/User:CleanupWorklistBot/lists/' . urlencode($filesafe_project);

		$csvurl = $this->urlpath . 'csv/' . urlencode($filesafe_project) . '.csv';
		...
		$alphaurl = $this->urlpath . 'alpha/' . urlencode($filesafe_project) . '.html';
		...
		echo 'wikipage size = ' . strlen($output) . "\n";
