...
class CleanupWorklistBot
{
    ...
    static public function writeWikiStats($tools_host, $user, $pass)
    {
        ...
        $sth = $dbh_tools->prepare("SELECT * FROM history WHERE project = ? ORDER BY time DESC LIMIT 1");
        ...
        foreach ($results as $row) {
        	...
        	$line = "<section begin=\"$project\" />Of the $project_pages articles in this project $cleanup_pages or $artcleanpct% are marked for cleanup, with $issue_count issues in total.<section end=\"$project\" />";
        	$wikitext .= $line;
        }
