...
//         }
       ...
       $creators, $deletedexistingcnt)
    {
        ...
        $deletedurl = "https://bambots.brucemyers.com/DeletedNewPages.php?project=" . urlencode($rulename) . "&month=$currentmonth";
    	$linecnt = 0;
    	...
    	if (isset($ruleset->options['MaxDays'])) {
    	    $display_max_days = (int)($ruleset->options['MaxDays']);
    	    if ($display_max_days < 7) $display_max_days = 7;
    	}
    	
    	$output = "<noinclude>__NOINDEX__\n{{fmbox|text= This is a new articles list for a Portal or WikiProject. See \"What links here\" for the Portal or WikiProject. See [[User:AlexNewArtBot|AlexNewArtBot]] for more information.}}</noinclude>This list was generated from [[User:AlexNewArtBot/{$rulename}|these rules]]. Questions and feedback [[User talk:Bamyers99|are always welcome]]! The search is being run daily with the most recent ~$display_max_days days of results. ''Note: Some articles may not be relevant to this project.''

[[User:AlexNewArtBot/{$rulename}|Rules]] | [[User:InceptionBot/NewPageSearch/{$rulename}/log|$logerror]] <includeonly>| [[$resultpage|Results page]] (for watching) </includeonly>| <span class='plainlinks'>[$deletedurl Deleted articles]</span>| Last updated: {{subst:CURRENTYEAR}}-{{subst:CURRENTMONTH}}-{{subst:CURRENTDAY2}} {{subst:CURRENTTIME}} (UTC)
