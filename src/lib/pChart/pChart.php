...
class pChart
{
	...
	function __construct($XSize,$YSize)
	{
		...
		$this->FontDir = dirname(__FILE__) . DIRECTORY_SEPARATOR . 'Fonts' . DIRECTORY_SEPARATOR;
		$this->ErrorFontName = $this->FontDir . 'pf_arma_five.ttf';
		$this->setFontProperties($this->FontDir . 'tahoma.ttf',8);
	}
	...
	function drawScale($Data, $DataDescription,$ScaleMode,$R,$G,$B,$DrawTicks=TRUE,$Angle=0,$Decimals=1,$WithMargin=FALSE,$SkipLabels=1,$RightScale=FALSE, $ID = 1)
	{
		...
		{
			if (isset($DataDescription['Values'][0]) && is_numeric($DataDescription['Values'][0]))
			{
	...
	function drawGrid($LineWidth,$Mosaic=TRUE,$R=220,$G=220,$B=220,$Alpha=100)
	{
		...
		for($i=1;$i<=$ColCount;$i++)
		{
			...
			$this->drawDottedLine(floor($XPos),$this->GArea_Y1,floor($XPos),$this->GArea_Y2-1,$LineWidth,$R,$G,$B);
			$XPos = $XPos + $this->DivisionWidth;
		}
	}
	...
	function drawTitle($XPos,$YPos,$Value,$R,$G,$B,$XPos2=-1,$YPos2=-1,$Shadow=FALSE)
	{
		...
		if (is_null($XPos)) {
			$Position  = imageftbbox($this->FontSize,0,$this->FontName,$Value);
			$TextWidth = $Position[2]-$Position[0];
			$XPos      = floor(( $this->XSize - $TextWidth ) / 2 );
		}
	...
	function drawPlotGraph($Data, $DataDescription,$BigRadius=5,$SmallRadius=2,$R2=-1,$G2=-1,$B2=-1,$Shadow=FALSE,$PointHasColor=FALSE)
	{
		/* Validate the Data and DataDescription array */
		...
		foreach ( $DataDescription['Values'] as $Key2 => $ColName )
		{
			...
			{
				if ($PointHasColor) {
					list($Value, $R, $G, $B) = explode(',', $Data[$Key][$ColName]);
				} else {
					$Value = $Data[$Key][$ColName];
				}
	...
	function drawLineGraph($Data, $DataDescription,$SerieName='')
	{
		...
		foreach ( $DataDescription['Values'] as $ColName )
		{
			...
			if ( $SerieName == '' || $SerieName == $ColName )
			{
				...
				$XStart = $XLast = -1;
				$YPos = -1;
				$PrevValue = $PrevValue2 = false;
				foreach ( $Data as $Key => $Values )
				{
					if ( isset($Data[$Key][$ColName]))
					{
						$Value = $Data[$Key][$ColName];
						if (is_numeric($Value)) $YPos = $this->GArea_Y2 - (($Value - $this->VMin) * $this->DivisionRatio);
						...
						if (! is_numeric($Value)) { $XStart = -1; }
						if ($XStart != -1)
							$this->drawLine($XStart,$YLast,$XPos,$YPos,$this->Palette[$ColorID]['R'],$this->Palette[$ColorID]['G'],$this->Palette[$ColorID]['B'],TRUE);
						elseif (is_numeric($PrevValue) && ! is_numeric($PrevValue2)) {
							//$this->drawAntialiasPixel($XLast,$YLast,$this->Palette[$ColorID]['R'],$this->Palette[$ColorID]['G'],$this->Palette[$ColorID]['B']);
							$this->drawFilledCircle($XLast+1,$YLast+1,1,$this->Palette[$ColorID]['R'],$this->Palette[$ColorID]['G'],$this->Palette[$ColorID]['B']);
						}
				...
				if (is_numeric($PrevValue) && ! is_numeric($PrevValue2)) {
					//$this->drawAntialiasPixel($XLast,$YLast,$this->Palette[$ColorID]['R'],$this->Palette[$ColorID]['G'],$this->Palette[$ColorID]['B']);
					$this->drawFilledCircle($XLast+1,$YLast+1,1,$this->Palette[$ColorID]['R'],$this->Palette[$ColorID]['G'],$this->Palette[$ColorID]['B']);
				}
				$GraphID++;
	...
	function validateDataDescription($FunctionName,$DataDescription,$DescriptionRequired=TRUE)
	{
		...
		if ( $DescriptionRequired )
		{
			if (!isset($DataDescription['Description']))
			{
				...
				foreach($DataDescription['Values'] as $Value)
				{
