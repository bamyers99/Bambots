...
while (! feof($hndl)) {
    ...
    if (! in_array('Q16521', $instanceofs)) continue; // taxon
    
    $taxonnames = $item->getStatementsOfType(WikidataItem::TYPE_TAXON_NAMES);
    if (empty($taxonnames)) continue;
    
    fwrite($whndl, "$qid\t{$taxonnames[0]}\n");
}
